...
string randomEmail(const string& first, const string& last) {
	const vector<string> domains = { "gmail.com", "yahoo.com", "outlook.com",
	"example.com" };
	return first + "." + last + "@" + domains[rand() % domains.size()];
}
// randomly generate categories
string randomCategory() {
	const vector<string> categories = { "Technology", "Sports", "Music", "Food",
	"Travel", "Education" };
...
vector<User> readCSV(const string& fileName) {
	...
	while (getline(file, line)) {
		...
		// Safely convert numPostsStr to integer
		try {
			user.numPosts = stoi(numPostsStr);
		}
		catch (const invalid_argument& e) {
			cerr << "Invalid number of posts for user " << userName << ": " << numPostsStr << endl;
			user.numPosts = 0; // Default value in case of an error
		}
		catch (const out_of_range& e) {
			cerr << "Number of posts out of range for user " << userName << ": " << numPostsStr << endl;
			user.numPosts = 0; // Default value in case of an error
		}

		// Push back to the users vector after successful assignment and error handling
...
vector<User> generateUsers(int n) {
	...
	for (int i = 0; i < n - numExist; ++i) {
		...
		user.userName = randomName() + "_" + user.firstName + user.lastName +
			to_string(rand() % 100);
